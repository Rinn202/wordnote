    ...
    return (
        <Modal open={open} onClose={onClose} title="📂 보드 저장 / 불러오기" width={480}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <button 
                    onClick={onSave} 
                    style={{ padding: '10px 16px', background: '#0f172a', color: '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}
                >
                    💾 현재 보드 저장
                </button>
                ...
                {boards.map(b => (
                    <div
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 10, border: '1.5px solid #e2e8f0', cursor: 'pointer', background: '#f8fafc', transition: 'border-color 0.15s' }}
